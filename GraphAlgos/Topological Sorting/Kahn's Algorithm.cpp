...
ostream &operator<<(ostream &ostream, const pair<T, V> &c)
{
...
void solve(int testcase)
{
   ...
   vector<int> out(n + 1, 0), vis(n + 1, 0), indeg(n + 1, 0);
   f(i, 1, n + 1) for(auto &it: adj[i]) indeg[it]++; 
   set<int> st;
   // use set for lexographically smallest toposort
   f(i, 1, n + 1) if(!indeg[i]) st.insert(i);
   vi topo;
   while(!st.empty())
   {
      int u = *st.begin(); st.erase(st.begin());
      for(auto &it: adj[u]) if(--indeg[it] == 0) st.insert(it);
      topo.push_back(u);
   }
   if(topo.size() != n) cout << "IMPOSSIBLE" << nl; // CYCLE DETECTED
   else cout << topo << nl;
}
