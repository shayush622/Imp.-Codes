...
void solve(int testcase)
{
   ...
   auto dfs = [&](int node, auto &&dfs) -> void
   {
      for(auto &u: adj[node])
      {
         ...
         else if(dep[u] < dep[node]) // back-edge upwards
            dp[node]++;
         else if(dep[u] > dep[node]) // back-edge downwards
            dp[node]--;
      }
      dp[node]--; // compensation for (the parent edge is not a back-edge)
      if(dep[node] > 1 && dp[node] == 0) bridges++;
   };
   dfs(1, dfs);
   cout<<bridges<<nl;
